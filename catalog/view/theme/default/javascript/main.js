...
Number.prototype.formatMoney = function(c, d, t){
    var n = this,
        cn = isNaN(c = Math.abs(c)) ? 2 : c,
        dn = d === undefined ? "." : d,
        tn = t === undefined ? "." : t,
        s = n < 0 ? "-" : "",
        i = parseInt(n = Math.abs(+n || 0).toFixed(cn)) + "",
        j = (j = i.length) > 3 ? j % 3 : 0;
    return s + (j ? i.substr(0, j) + tn : "") + i.substr(j).replace(/(\d{3})(?=\d)/g, "$1" + tn) + (cn ? dn + Math.abs(n - i).toFixed(cn).slice(2) : "");
};
